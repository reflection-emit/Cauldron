# ConcurrentList(*T*).ToList Method 
 _**\[This is preliminary documentation and is subject to change.\]**_

Creates a List(T) from an <a href="T_Couldron_Collections_ConcurrentList_1">ConcurrentList(T)</a>.

**Namespace:**&nbsp;<a href="N_Couldron_Collections">Couldron.Collections</a><br />**Assembly:**&nbsp;Couldron (in Couldron.dll) Version: 1.0.0.0 (1.0.0.0)

## Syntax

**C#**<br />
``` C#
public List<T> ToList()
```


#### Return Value
Type: List(<a href="T_Couldron_Collections_ConcurrentList_1">*T*</a>)<br />A List(T) that contains elements from the input sequence.

## See Also


#### Reference
<a href="T_Couldron_Collections_ConcurrentList_1">ConcurrentList(T) Class</a><br /><a href="N_Couldron_Collections">Couldron.Collections Namespace</a><br />