...
namespace UnitTest_BasicInterceptorTests
{
    ...
    public class Field_Interception_Code_Validation_Tests
    {
        ...
        {
            var generic = new GenericTypeWithGenericField<long, TestClass, TestClass, List<string>>();
    ...
    public class GenericTypeWithGenericField<T, T2, T3, T4>
        where T2 : class
        where T3 : new()
        where T4 : IEnumerable
    {
        ...
        [TestPropertyInterceptor]
        private T2 fieldWithGenericSelectorClass;

        [TestPropertyInterceptor]
        private T3 fieldWithGenericSelectorNew;
