...
namespace Cauldron.XAML
{
    ...
    public sealed class MessageDialogEx : IMessageDialog
    {
        ...
        /// <param name="commands">An array of commands that appear in the command bar of the message dialog. These commands makes the dialog actionable.</param>
        /// <returns>An object that represents the asynchronous operation.</returns>
        public async Task ShowAsync(string title, string content, uint defaultCommandIndex, uint cancelCommandIndex, MessageBoxImage messageBoxImage, CauldronUICommandCollection commands) =>
            await this.navigator.NavigateAsync<MessageDialogViewModel>(new Func<Task>(() => Task.FromResult(0)), title, content, (int)messageBoxImage, defaultCommandIndex, cancelCommandIndex, commands);
