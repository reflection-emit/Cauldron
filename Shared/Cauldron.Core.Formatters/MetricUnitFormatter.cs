...
namespace Cauldron.Core.Formatters
{
    ...
    public sealed class MetricUnitFormatter : FormatterBase
    {
        ...
        public MetricUnitFormatter(CultureInfo cultureInfo) : base(cultureInfo)
        {
        }
        ...
        {
            // Origin: http://stackoverflow.com/questions/12181024/formatting-a-number-with-a-metric-prefix
            //         by Thom Smith
            ...
            if (format.StartsWith(metricKey))
            {
                ...
                if (!double.TryParse(arg?.ToString() ?? "0.0", out value))
                    value = Convert.ToDouble(arg.ToString());
