public const string Name = "XAML initializer for baml resources";
    ...
    resourceDictionaryMergerClass.CreateConstructor().NewCode().Context(x =>
    {
        ...
        foreach (var item in xamlThatRequiredInitializers)
        {
            ...
            x.Call(resourceDictionaryInstance, resourceDictionary.SetSource,
                 x.NewCode().NewObj(uri.Ctor, "pack://application:,,,/" + System.IO.Path.GetFileNameWithoutExtension(builder.Name) + ";component/" + item.Item)); // TODO -Need modification for UWP)
