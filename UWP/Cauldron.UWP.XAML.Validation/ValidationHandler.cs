    ...
    public sealed class ValidationHandler : DisposableBase
    {
            ...
            lock (syncValidationRoot)
            {
                ...
                foreach (var validatorGroup in this.validators)
                {
                    foreach (var validator in validatorGroup)
                    {
                        ...
                        if (!string.IsNullOrEmpty(error))
                            this.AddError(validatorGroup.PropertyName, error);
                    }

                    this.RaiseErrorsChanged(validatorGroup.PropertyName);
                }
            }
        ...
        private void AddValidators()
        {
            lock (syncValidationRoot)
            {
                ...
                {
                    foreach (ValidationBaseAttribute attrib in property
                        .GetCustomAttributes(false)
                        .Where(x => (x as ValidationBaseAttribute) != null))
                    {
                        if (attrib == null)
                            continue;
